...
export default function MatchingsPage() {
    const [matchedUserList, setMatchedUserList] = useState<MatchedUser[] | undefined>([]); // 初期値を[]に変更
    const [error, setError] = useState<string | null>(null);

    useEffect(() => {
        async function fetchMatchedUserList() {
            try {
                const res = await fetch('/api/request/getMatchedRequests');
                const data = await res.json();
                console.log(JSON.stringify(data, null, 2)); // レスポンスを確認
    ...
    return (
        <div className="p-6 max-w-3xl mx-auto">
            <h1 className="text-2xl font-semibold mb-6">マッチング一覧</h1>

            {error && <p className="text-red-500 mb-4">{error}</p>}

            <ul className="space-y-4">
                {matchedUserList && matchedUserList.length === 0 ? (
